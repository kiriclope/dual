...
misc.add_argument('--save-weights', action='store_true', dest='save_weights',
                  help='Also save fold-averaged discriminant axes (raw neuron space) '
                       'per (mouse, stage, context, target) to weights_<dum>.pkl, for '
...
t0 = perf_counter()
X_mice, y_mice = [], []
weights_out, valid_masks = {}, {}   # filled only when --save-weights
    ...
    for stage in args.stages:
        ...
        for context in args.contexts:
            ...
            for target in args.targets:
                ...
                if args.save_weights and null_info.get('weights') is not None:
                    # (n_train_times, n_neurons_mouse) in raw neuron space
                    weights_out[(mouse, stage, context, target)] = null_info['weights']
