...
rows = [('DPA', 'performance', y.tasks == 'DPA'), ('GNG (Go+NoGo)', 'odr_perf', y.tasks != 'DPA'),
        ('Go only', 'odr_perf', y.tasks == go), ('NoGo only', 'odr_perf', y.tasks == nogo)]
for name, col, mask in rows:
    d = MP._perf_delta_by_sample(col, mask)
    rho, p, n, mx, my = MP._panelC_coupling(d)
    r, pr = pearsonr(mx, my)
    print(f'{name:14s} n={n}  Spearman rho={rho:+.2f} p={p:.3f}   Pearson r={r:+.2f} p={pr:.3f}   '
