...
# dPCA components are grouped by marginalisation (not variance-ordered), so an
# EVR scree line is meaningless — use a bar chart labelled/coloured by marginal
try:
    MARGS = pkl_load(f'pseudo_marglabels_{DUM}', path=DATA)
...
if MARGS is not None:
    # dPCA: variance explained per MARGINALISATION (sum its q component axes)
    uniq = list(dict.fromkeys(MARGS))
    var_marg = np.array([evr_mean[[i for i, m in enumerate(MARGS) if m == u]].sum()
                         for u in uniq])
    cols = sns.color_palette('tab10', len(uniq))
    fig, ax = plt.subplots(figsize=(max(W, 0.7 * len(uniq)), W))
    ax.bar(range(len(uniq)), var_marg, color=cols)
    for i, v in enumerate(var_marg):
        ax.text(i, v, f'{v*100:.1f}%', ha='center', va='bottom', fontsize=6.5, color='0.3')
    ax.set_xticks(range(len(uniq)))
    ax.set_xticklabels(uniq, fontsize=7, rotation=20, ha='right')
    ax.set_xlabel('marginalisation')
    ax.set_title(f'dPCA — variance explained per marginalisation '
                 f'(q={n_comp // len(uniq)} axes each)', loc='left', fontsize=TITLE_FS)
else:
...
for k, ax in enumerate(axes):
    ...
    ax.set_ylabel(f'Weights {pc_label(k, PC_ID)}')
    ax.set_xlabel('Neuron loc (°)')
    ax.set_ylim([-z_lim, z_lim])
    ax.set_xticks([-180, -90, 0, 90, 180])
    ax.tick_params(labelsize=7)
plt.colorbar(sc, ax=axes[-1], label='Angle (°)', shrink=0.8)
fig.suptitle('Pseudo-PC loadings vs preferred direction', fontsize=TITLE_FS, y=1.01)
fig.tight_layout()
...
for ax, (a, b) in zip(axes, [(0, 1), (0, 2), (1, 2)]):
    ...
    ax.set_xlabel(f'{pc_label(a, PC_ID)} weight')
    ax.set_ylabel(f'{pc_label(b, PC_ID)} weight')
    ax.set_xlim(-z_lim, z_lim)
    ax.set_ylim(-z_lim, z_lim)
    ax.tick_params(labelsize=7)
plt.colorbar(sc, ax=axes[-1], label='Angle (°)', shrink=0.8)
fig.suptitle('Pseudo-PC loadings in weight space', fontsize=TITLE_FS, y=1.01)
fig.tight_layout()
...
for ax, df, ylab, pct in (
    ...
    if pct:
        ax.yaxis.set_major_formatter(mticker.PercentFormatter(xmax=1, decimals=0))
axes[0].legend(fontsize=6.5, frameon=False, bbox_to_anchor=(1, 1), loc='upper left')
fig.suptitle('Pseudo-PC loading energy per mouse', fontsize=TITLE_FS, y=1.01)
fig.tight_layout()
