        ...
        if SCALE is not None:
            for day in range(1, options_mouse["n_days"] + 1):
                ...
                if SCALE == 'mad':
                    ...
                    lo, hi = np.nanpercentile(std_.ravel(), [5, 95])
                    std_ = np.clip(std_, lo, hi)
                ...
                if SCALE == 'very_mad':
                    ...
                    lo, hi = np.nanpercentile(std_.ravel(), [25, 75])
                    std_ = np.clip(std_, lo, hi)

                if SCALE == 'std_trial':
                    std_  = np.nanstd(X[idx0], axis=0, ddof=0, keepdims=True)
                    lo, hi = np.percentile(std_.ravel(), [5, 95])
                    std_ = np.clip(std_, lo, hi)
...
#+begin_src jupyter-python
X_all, y_all, mouse_slices = build_padded_X(options, n_neurons_total=3319, SCALE=SCALE)
print(X_all.shape, y_all.shape)
#+end_src

#+RESULTS:
#+begin_example
JawsM01 (768, 184, 84) (768, 16)
JawsM06 (1152, 201, 84) (1152, 16)
JawsM12 (960, 423, 84) (960, 16)
JawsM15 (1152, 693, 84) (1152, 16)
...
pkl_save(X_all, 'X_all_%s' % dum, path="../data/pca")
pkl_save(y_all, 'y_all_%s' % dum, path="../data/pca")
pkl_save(mouse_slices, 'mouse_slices', path="../data/pca")
#+end_src

#+RESULTS:
: saving to ../data/pca/X_all_center.pkl
: saving to ../data/pca/y_all_center.pkl
...
X_all = pkl_load('X_all_%s' % dum, path="../data/pca")
y_all = pkl_load('y_all_%s' % dum, path="../data/pca")
mouse_slices = pkl_load('mouse_slices', path="../data/pca")
#+end_src

#+RESULTS:
: loading from ../data/pca/X_all_.pkl
: loading from ../data/pca/y_all_.pkl
...
def cv_pca(
    ...
    m_clean = (y.laser == 0) & (y.learning==learning) & (y.performance==1) # & ((y.tasks=='DPA') | (y.odr_perf==1))
    Xc = X[m_clean]
    ...
    if np.any(m_pert):
        ...
        if descale:
            Zp = Zp * scaler.scale_ + scaler.center_
    ...
    for train_idx, test_idx in splitter:
        ...
        if descale:
            Z_test = Z_test * scaler.scale_ + scaler.center_
...
X_cv, y_cv, w_cv, w_all, evr_cv = cv_pca(X_pca, y_pca, pca, folds, factors, epoch, mouse_slices=mouse_slices, mouse_gain_mode=None, scale=scale, scale_test=1, descale=0)
#+end_src

#+RESULTS:
: cv_pca: 100% 50/50 [02:03<00:00,  2.47s/it]
...
#+RESULTS:
: (27693, 84, 10) (27693, 18) (10, 3319) (10, 3319) (50, 10)
...
#+RESULTS:
: (27693, 10, 84) (27693, 18) (10, 3319)

#+begin_src jupyter-python
print(evr_meta.shape)
evr_mean = np.array(evr_meta, dtype=float).mean(0)
evr_sem = 1.96 * np.std(np.array(evr_meta, dtype=float), axis=0, ddof=1) / np.sqrt(evr_meta.shape[0])
print(evr_mean.shape, evr_sem.shape)
#+end_src
...
#+begin_src jupyter-python
n_pcs = np.arange(1, evr_meta.shape[1]+1)
...
# for i in range(evr_meta.shape[0]):
#     plt.plot(n_pcs, evr_meta[i], '-', alpha=0.5)
...
plt.savefig('figures/meta/evr_ratio_%s.svg' % dum)
plt.show()
#+end_src

#+RESULTS:
[[file:./figures/pca/figure_28.png]]
...
#+RESULTS:
: saving to ../data/pca//meta_traj_POST_GNG_center_10.pkl
: saving to ../data/pca//meta_labels_POST_GNG_center_10.pkl
: saving to ../data/pca//meta_weights_POST_GNG_center_10.pkl
: saving to ../data/pca//meta_evr_POST_GNG_center_10.pkl
...
#+RESULTS:
: loading from ../data/pca//meta_traj_TEST_std_trial_10.pkl
: loading from ../data/pca//meta_labels_TEST_std_trial_10.pkl
: loading from ../data/pca//meta_weights_TEST_std_trial_10.pkl
: loading from ../data/pca//meta_evr_TEST_std_trial_10.pkl
...
idx_correct =  (y_meta.learning=='Expert') & (y_meta.performance==1)
idx_mouse = True
if i_mouse !=-1:
...
plt.savefig('./figures/meta/pca_traj_%s.svg' % dum)
plt.show()
#+end_src
...
#+begin_src jupyter-python
fig, ax = plt.subplots(nrows=1, ncols=n_comp, figsize=(n_comp*width, height), sharey=0)
...
for i in range(len(pair)):
    mask = (y_meta.tasks==y_meta.tasks.unique()[i]) & (y_meta.laser==laser) & idx_mouse & idx_correct & (y_meta.tasks!='DPA') & (y_meta.odr_perf==1)
    X_sel = X_meta[mask]
...
plt.savefig('./figures/meta/pca_traj_%s_tasks.svg' % dum)
plt.show()
#+end_src
...
for i in range(2):
    ...
    X_sel = X_meta[mask] * 10         # Subselect rows
...
plt.savefig('./figures/meta/pca_traj_%s_sample.svg' % dum)
plt.show()
#+end_src
...
plt.savefig('./figures/meta/pca_traj_%s_licks.svg' % dum)
plt.show()
#+end_src
...
for i in range(2):
    mask = (y_meta.test_odor==i) & (y_meta.laser==0) & idx_mouse & idx_correct
    X_sel = X_meta[mask] * 10
    X_avg = X_sel.mean(0)    # Mean over trials/samples, shape: (n_comp, n_time)
    X_sem = X_sel.std(0) / np.sqrt(X_sel.shape[0])  # SEM
...
plt.savefig('./figures/meta/pca_traj_%s_test.svg' % dum)
plt.show()
#+end_src
...
plt.savefig('./figures/meta/pca_traj_%s_2D.svg' % dum)
plt.show()
#+end_src
...
assert isinstance(y_meta, pd.DataFrame)
assert X_meta.shape[0] == len(y_meta)

pcs = [0, 1, 2]
pc_names = [f"PC{p+1}" for p in pcs]

# ---- build trial table + filter ----
df = y_meta.reset_index(drop=True).copy()

# keep only selected PCs: (n_trials, 3, n_time); store per-trial arrays
df["traj"] = list(X_meta[:, pcs, :]*10)
...
plt.savefig('./figures/meta/pca_scatter_%s.svg' % dum)
...
plt.savefig('./figures/meta/pca_scatter_choice_%s.svg' % dum)
...
plt.savefig('./figures/meta/pca_scatter_test_%s.svg' % dum)
...
axs[-1].axvline(45)
axs[-1].axvline(225)
plt.savefig(f'./figures/meta/pca_weights_{dum}_bin.svg')
plt.show()
#+end_src
...
def rebuild_padded_X(options, n_neurons_total=3319, SCALE=None):
    X_list = []
    y_list = []
    mouse_slices = {}
        ...
        if SCALE is not None:
            for day in range(1, options_mouse["n_days"] + 1):
                ...
                if SCALE == 'mad_trial':
                    ...
                    lo, hi = np.nanpercentile(std_, [5, 95])
                    std_ = np.clip(std_, lo, hi)
...
#+RESULTS:
: (28, 4) (28, 4)
: [-0.06737988 -0.01388889 -0.0396021   0.00694444 -0.17252066 -0.02777778
:  -0.17067931 -0.06944444 -0.03733766  0.04166667 -0.05587748 -0.00694444
:  -0.15799916  0.08333333 -0.12211829 -0.02777778 -0.23317643  0.0625
:  -0.12399194  0.04166667 -0.22718254 -0.02083333 -0.1447498  -0.01388889
:  -0.01176575 -0.02777778 -0.01942686  0.04861111]
...
#+RESULTS:
: (28, 4) (28, 4)
: [-0.0574187   0.01041667 -0.02349727  0.07291667 -0.08366935 -0.02083333
:  -0.05637376  0.          0.03311258  0.02083333 -0.03125     0.05208333
:  -0.06242079  0.         -0.0412756   0.         -0.03478145  0.02083333
:   0.03151151  0.0625      0.04096964  0.07291667  0.05056818  0.0625
:  -0.02957529  0.          0.02112907  0.03125   ]
...
: 8   JawsM01          0.0  -0.037338   0.033113  -0.243936  -0.155345
: 11  JawsM01          1.0  -0.006944   0.052083  -0.239301  -0.291244
: 10  JawsM01          1.0  -0.055877  -0.031250  -0.195449  -0.093780
: 9   JawsM01          0.0   0.041667   0.020833  -0.122028  -0.052632
: 15  JawsM06          1.0  -0.027778   0.000000   0.112363  -0.178344
...
for i, delta_perf in enumerate(['delta_dpa', 'delta_odr']):
    ...
    annotation = f"Pearson r = {corr:.2f}\np-value = {p_value:.3f}"
    ax[i].annotate(annotation, xy=(.65, 0.15), xycoords='axes fraction', fontsize=14,
                backgroundcolor='None', verticalalignment='bottom', horizontalalignment='left',
...
#+RESULTS:
[[file:./figures/pca/figure_75.png]]
...
#+RESULTS:
[[file:./figures/pca/figure_78.png]]
...
#+RESULTS:
[[file:./figures/pca/figure_82.png]]
...
#+RESULTS:
[[file:./figures/pca/figure_90.png]]
...
#+RESULTS:
[[file:./figures/pca/figure_92.png]]
...
#+RESULTS:
[[file:./figures/pca/figure_94.png]]
...
#+RESULTS:
[[file:./figures/pca/figure_96.png]]
...
#+RESULTS:
[[file:./figures/pca/figure_100.png]]
