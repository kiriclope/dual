...
def remove_ci_subspace(X, y, mouse_slices, q=2, factors=('odor_pair', 'tasks'),
    ...
    epoch : array-like of int or None — time window the CI directions are
        estimated from (None = whole timeline).  Match it to the fit epoch so the
        directions target the common mode relevant to that window (e.g. the
        test-evoked response for a TEST fit, not the delay ramp).
    """
    if q <= 0:
        return X
    if isinstance(factors, str):
    ...
    for mouse, sl in mouse_slices.items():
        ...
        R = np.nan_to_num(R)
        if epoch is not None:
            R = R[:, epoch]                              # estimate dirs in this window
