...
generalization has revealed in primate prefrontal cortex and hippocampus [Bernardi 2020]. Here
...
code is conjunctive, in that it anti-generalizes across sample, so the high-dimensional
...
makes the factorization legible at the level of single cells.
...
observations fit a protective function. The behavioral cost of the distractor is an evoked lick
...
trained animal leaves behavior and the fidelity of the code intact but shifts the code's
...
trades the two tasks against each other (Fig. 6g–i). The slow process finds the factorized
...
Composition, in this system, is not construction. mPFC brings a factorized, shared subspace to
