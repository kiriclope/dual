...
> **2026-09-08 (later) — choice-decoder TRIAL SETS.** Fig 3c–e per-mouse choice classes (lick vs no-lick at the test) now pool ALL trial types (`--dpachoice` = former DPA-only); Fig 4a's choice side reads the DUAL trials only (Go + NoGo, the same trials as its Go/NoGo side; `--dpaact` = former DPA-only), in the pooled matrix and the per-mouse scatter. Fig 3c choice brackets ∗ (p = .012); Fig 4a per-animal convergence is a trend (|cos| and cross-decode each up 6/9, Wilcoxon p = .20). Details: memory `project_dimensionality.md` / `project_overlaps_main_native.md`; draft banner v12.21.
