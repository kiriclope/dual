...
> ρ +0.05 p .20 — window CORRECTED 2026-09-16 to 5.5–7.0 s file time = imaging 7.5–9.0 s, licks on 6.9% of trials; covariate β +0.06 p .28); coupling on no-lick trials ρ −0.80 p .009; this re-run's all-trial coupling is
...
> **2026-09-09 — Fig 4c arms.** Panel c keeps the distractor-free DPA arm (dual / all trials give ρ −0.63 p .067, stated in the text; `--dualperf`); the NoGo-only arm's positive trend is a CEILING EFFECT (`exp_push_nogo_ceiling.py`: naïve NoGo predicts ΔNoGo ρ −0.87; partial corr with the push +0.27 p .49) — no cost, no benefit. Trial sets are named in the Fig 4 panel titles.
...
- 2026-08-31 (later): Fig 4 caption rewritten to the house caption standard (claim+roadmap
  title; per panel what/how/shows/stats; CODE DEPTH defined in the title; cross-refs to the opto
  figure's K/L "position not fidelity" (Fig 6 since 2026-09-01, Fig 5 at the time; Fig 5 is now
  reserved for the modelling figure); see memory feedback_caption_style) — numbers unchanged.
...
- Written into Results §4 (draft v12.10) together with the disclosed dependences: per-stage axis + logistic
  decoder required (L1 ρ = −0.73 ★, LDA n.s.; ED 4b — citation-order numbering since 2026-09-15), and on ONE axis fitted to both stages (run_overlaps --pool-stages, held-out) it REVERSES as a trend (ρ = +0.65 p .058; ED 4c, 2026-09-15) — the coupling is a property of the stage-specific readouts; lick covariate leaves it unchanged (ED 4d, CCGD depth), no replication on the dPCA axis,
  push significant only in evoked-s.d. / whole-trial-s.d. units, a trend (p .10) in the raw units Fig 4 uses (ED 4a).
