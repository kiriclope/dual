...
> laser-vector), so the ED robustness panels lag the main figures. _(2026-09-15: old ED 5 → ED 3 → ED 4 after the citation-order renumbering; old ED 6 → ED 2 → ED 1. ED 4c,d now come from the CCGD
> pipeline — `run_overlaps.py --pool-stages`, `exp_common_axis_ccgd.py`, `exp_lick_control_ccgd.py` — not from these
> scripts' held-out re-fits, which are gallery-only.)_
> _(ED 4d is now TRIAL-level: `run_overlaps.py --scaler none --targets choice --contexts all --tag trial` (8.1 min; the labels
> carry a `trial` column since 2026-09-15, folds seeded) then `exp_lick_control_trial.py` (0.1 min) → `ed3_cache['lick_trial']`;
> `exp_lick_control_ccgd.py` (mouse × sample × stage) is superseded, kept for the gallery with its .mat sample mapping fixed.)_
...
| `fig_overlaps_story_main.py` | `python fig_overlaps_story_main.py [--panels 8\|4] [--all-trials] [--stability B]` | **Composite STORY figure** (overlaps analog of the dPCA story fig): §3 8-panel rank-2 gain-modulated flow grid (autonomous/A/B/Go/NoGo/Cue/C/D) + §4 Naive vs Expert DPA autonomous flow + per-mouse push slopegraph + sample-sep control. **Per-regime decoders** (`REG_EPOCH`): autonomous+A/B@**DELAY** (A/B pooled with autonomous, wells on the WM landscape — NOT stim, whose choice axis is garbage); Go/NoGo@**RESPONSE** (lick axis; only there does Go↑/NoGo↓ read); Cue@**CUE**; **C/D on a MIXED plane sample@TEST × choice@RESPONSE** (what makes the opposite diagonals — sample@test gives A−/B+ x-sep, choice@resp gives match/mismatch y-sep; `codes(sample_bins, choice_bins)` allows the split axes). **Per-group ridge**, **10×5 repeated CV** (`RepeatedKFold`, per-fold means precomputed). Canonical correct mask `performance & (DPA\|odr_perf)`. Root-found FPs. `--all-trials`→`_all`, `--panels 4`→`_p4`. **`--stability B`** = mouse-subsample (7/9, B×) robustness of the §3 features → input structure 90–100% robust, **autonomous bistability only 43% (fragile — keep soft)**. Full ref: overview.md "Composite story figure". A single fixed dPCA-style plane ERASES the structure — don't re-attempt. | `figures/overlaps/story/{png,svg}/fig_overlaps_story_main{,_p4,_all}.*` |
| `fig_ed_behavior.py` | `python fig_ed_behavior.py [--nocap]` (1 min) | **ED 1**: lick rasters/PSTHs from the .mat lick times (file time + 2.0 s = imaging), delay-lick rate per mouse, per-animal learning curves, Fig 1g GEE with trial-history covariates | `figures/ed/{png,svg}/ed_fig1` |
| `fig_ed_opto_validation.py` | `python fig_ed_opto_validation.py [--nocap]` | **ED 8** PLACEHOLDER: six labelled boxes for author-supplied histology, placements, FOVs, laser parameters | `figures/ed/{png,svg}/ed_fig7` |
| `fig_ccgp.py` | `python fig_ccgp.py --canon --alltrials` (19 min; `--canon` alone = correct-only, 2 min) | per-mouse CCGP of the four codes (sample / GNG / test / choice) on the canonical windows (sample & GNG 33–38, test & choice 54–62), ALL laser-off trials since 2026-09-15 — the cache `permouse_ccgp_cache_canon_all.pkl` is what `pca/fig_ed2_plane.py` draws as ED 1b | `figures/overlaps/ccgp/permouse_ccgp_cache_canon_all.pkl` |
| `run_overlaps.py … --tag trial` | `python run_overlaps.py --scaler none --targets choice --contexts all --tag trial` (8.1 min) | the choice decoder re-run with SEEDED folds; since 2026-09-15 every run's labels carry a within-session `trial` column (imaging order = .mat AllTrials order), so held-out rows can be joined to behavioural trials | `data/overlaps/{X,labels}_…_raw_targets_choice_trial.pkl` |
| `exp_coupling_variants.py` | `python exp_coupling_variants.py` (0.4 min) | Fig 4c coupling under l2 / l1 / LDA decoders (one `main_panels` subprocess per variant) → ED 4b | `figures/overlaps/controls/coupling_variants_cache.pkl` |
| `exp_common_axis_ccgd.py` | `python exp_common_axis_ccgd.py` (0.7 min; needs the `--pool-stages` tensor) | push + coupling on the per-stage axes vs ONE pooled-stage axis per mouse (held-out) → ED 4c; the coupling reverses on the pooled axis | `ed3_cache.pkl['common_ccgd']` |
| `exp_lick_control_trial.py` | `python exp_lick_control_trial.py` (0.1 min; needs the `--tag trial` tensor) | TRIAL-level lick control: per-mouse Spearman(depth, late-delay lick rate), push by trial set, coupling on no-lick trials → ED 4d | `ed3_cache.pkl['lick_trial']` |
| `exp_lick_control_ccgd.py` | `python exp_lick_control_ccgd.py` (0.1 min) | mouse × sample × stage lick control on the canonical tensor (superseded by the trial-level one; .mat `sample` 1/2 = A/B) | `ed3_cache.pkl['lick_ccgd']` (gallery only) |
...
> push; panel D coupling = per-mouse n=9 Spearman ρ≈−0.83 p≈.005 ★ DPA-specific — the earlier
> "push↔coupling tension" was a panel-D LMM artifact, resolved 2026-07-18). The axes/stats below describe
> the earlier locked build and are superseded.
